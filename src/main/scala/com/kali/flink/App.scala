...
object App {
  def main(args: Array[String]): Unit = {
    ...
    if (SystemUtils.IS_OS_WINDOWS) {
      ...
      env.setStateBackend(new FsStateBackend("hdfs://acquirel:9000/flink-checkpoint/checkpoint"))
    }
    //===========类型2:建议参数===========
...
//    env.getCheckpointConfig.setTolerableCheckpointFailureNumber(10) //默认值为0，表示不容忍任何检查点失败
    ...
    val kafkaSource: FlinkKafkaConsumer[String] = new FlinkKafkaConsumer[String]("clickLog", new SimpleStringSchema, props)
...
//    )
    ...
    val clickLogWideDS: DataStream[ClickLogWide] = DataToWideTask.process(messageDS)
